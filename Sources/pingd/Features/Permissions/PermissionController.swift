...
struct PermissionController: RouteCollection, @unchecked Sendable {
    ...
    func boot(routes: any RoutesBuilder) throws {
        routes.get("users", ":username", "permissions", use: list)
        routes.post("users", ":username", "permissions", use: create)
        routes.delete("permissions", ":id", use: delete)
    }
...
struct CreatePermissionRequest: Content {
    let scope: PermissionScope
    let accessLevel: AccessLevel
    let topicPattern: String
}
